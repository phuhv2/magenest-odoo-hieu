...
class PlanSaleOrder(models.Model):
    ...
    def btn_send(self):
        ...
        if self.state == 'new':
            if self.order_line.approver:
                ...
                # nếu không thêm người phê duyệt sẽ hiển thị thông báo
                raise UserError('This plan does not have any approvers.')
        else:
            # kiểm tra nếu đã gửi cho người này rồi, thì không gửi nữa
    ...
    def btn_confirm_approve(self):
        ...
        if self.can_confirm == 'yes':
            ...
            # can_confirm chưa yes, nên load lại trang
    ...
    # khi plan sale ở trạng thái approve hoặc send thì employee không được phép xóa
    def unlink(self):
        for r in self:
